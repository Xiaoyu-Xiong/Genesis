...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      ...
      echo "Usage: $0 [--model MODEL] [--critic-model MODEL] [--reasoning-effort EFFORT] [--critic-reasoning-effort EFFORT] [--max-parallel N] [--gpu|--cpu|--backend cpu|gpu] [--run-root PATH]" >&2
      exit 2
      ;;
  esac
done
