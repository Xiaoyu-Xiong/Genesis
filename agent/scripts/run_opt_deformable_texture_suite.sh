...
cat > "$TASK_FILE" <<'CASES'
textured_duck_plate_press|Create a scene where around 10 identical medium-sized soft rubber ducks with bright toy-like texture are compressed by a descending rigid plate, showing visible squashing and texture distortion, render 10s behavior

striped_gel_block_squeeze|Create a scene where several identical medium-sized soft striped gel blocks are squeezed between moving rigid paddles so the colored stripes make stretching and compression easy to see, render 10s behavior
...
beach_ball_gate_sweep|Create a scene where identical soft striped beach balls are pushed through a narrowing rigid gate so contact-rich deformation and texture stretching are clearly visible, render 10s behavior
...
monster_ring_compression|Create a ring of identical soft monster toys with colorful textured surfaces and let a rigid plate descend into the ring so they compress, buckle, and recover over 10s
