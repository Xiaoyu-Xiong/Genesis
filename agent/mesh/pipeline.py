...
def generate_meshy_mesh_from_text(
    *,
    ...
    raw_manifold_result = time_stage(
        profile_sec,
        "raw_manifold_check",
        lambda: run_mesh_manifold_check(pipeline_source_mesh_path),
    )
    ...
    repair_result, repair_attempts, final_manifold_result = run_repair_pipeline(
        mesh_path=pipeline_source_mesh_path,
        output_dir=output_dir,
    ...
    metadata = {
        ...
        "texture_config": None if texture_config is None else texture_config.to_dict(),
        "pipeline_source_mesh_path": str(pipeline_source_mesh_path),
