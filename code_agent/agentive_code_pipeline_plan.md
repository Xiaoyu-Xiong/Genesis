...
mesh asset request 也由 Planner 决定是否调用。Planner 可以先 `start_mesh_assets`，然后在 asset job 后台运行
时并行唤醒不依赖 manifest 的 writer。只有 module contract 里声明了 `asset_dependencies` 或显式
`asset_manifest` 输入的 writer，会被 harness 要求等到 `wait_mesh_assets` 完成以后才能启动。生成出的 manifest
会直接进入 writer prompt，writer 只能根据 manifest 中的 canonical runtime path 使用 mesh，不再猜测文件路径。
