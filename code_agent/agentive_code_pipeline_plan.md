...
同一个 `spawn_workers` action 中的多个 role 会由 harness 并行启动，最大并行度由
`CONFIGS.harness.max_parallel_workers` 控制。Planner 负责决定哪些 role 可以放进同一批；存在依赖关系的
role 应拆成多轮 `spawn_workers`，让后续 Planner turn 在看到前置 worker 结果后再继续。
