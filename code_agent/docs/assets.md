...
The current concrete implementation is the mesh pipeline under `assets/mesh`. MJCF/XML assets should be generated by a
Codex XML worker when that worker is added to the Planner action library.
...
Genesis simulation. The future harness should execute this validation through the repository uv environment and record the result in the
Asset Manifest.
