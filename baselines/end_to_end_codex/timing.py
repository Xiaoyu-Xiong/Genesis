...
from baselines.end_to_end_codex.configs import runtime_defaults_dict


@dataclass(slots=True, frozen=True)
class BaselineTimingPlan:
...
def resolve_baseline_timing(
    *,
    ...
    sim_dt = _positive_float(execution_plan.get("sim_dt")) or float(defaults["sim_dt"])
    sim_substeps = _positive_int(execution_plan.get("sim_substeps")) or int(defaults["sim_substeps"])
    render_every_n_steps = _positive_int(execution_plan.get("render_every_n_steps")) or int(
        defaults["render_every_n_steps"]
    )
    render_res = _render_res(execution_plan.get("render_res")) or _render_res(defaults["render_res"]) or (640, 480)
    ...
    fps = planned_fps or int(defaults["render_fps"])
